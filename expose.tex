...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \tikzset{normal/.style={minimum width=2cm, minimum height=0.8cm, dashed, draw=black!50}}
    \def\x{0}
    \def\y{0}
    \def\xd{3}
    \def\yd{1}

    \node[normal] (chisel) at (\x,\y) {Chisel};
    \node[normal] (svlang) at (\x+\xd,\y) {SystemVerilog};
    \node[normal] (circt) at (\x+0*\xd,\y-1*\yd) {CIRCT};
    \node[normal] (yosys) at (\x+0*\xd,\y-2*\yd) {Yosys};
    \node[normal] (verilator) at (\x+1*\xd,\y-2*\yd) {Verilator};
  \end{tikzpicture}
  \caption{Open-source hardware design tool landscape}\label{fig:toolLandscape}
\end{figure}
